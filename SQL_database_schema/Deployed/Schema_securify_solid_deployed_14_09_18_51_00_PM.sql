...
CREATE TABLE IF NOT EXISTS `securify`.`users` (
  ...
  PRIMARY KEY (`userID`),
  UNIQUE INDEX `email` (`email` ASC, `userID` ASC))
ENGINE = MyISAM
...
CREATE TABLE IF NOT EXISTS `securify`.`registercar` (
  ...
  CONSTRAINT FK_PersonOrder FOREIGN KEY (userID) REFERENCES `securify`.`users`(`userID`),
  UNIQUE INDEX `registrationID_NEED_UNIQUE` (`registerationID` ASC),
...
CREATE TABLE IF NOT EXISTS `securify`.`data` (
  ...
  UNIQUE INDEX `data_logging_NEED_UNIQUE` (`measurementID` ASC),
...
CREATE TABLE IF NOT EXISTS `securify`.`joblist` (
  ...
  FOREIGN KEY (`userID`) REFERENCES `securify`.`registercar`(`userID`) ON DELETE CASCADE,
  FOREIGN KEY (`carID`) REFERENCES `securify`.`registercar`(`carID`) ON DELETE CASCADE,
  UNIQUE INDEX `data_logging_NEED_UNIQUE` (`jobID` ASC),
...
INSERT INTO `users` (`userID`, `fname`, `lname`, `email`, `phone`, `pass_hashed`, `pass_salt`, `role`, `gps_long`, `gps_lati`, `isOnline`, `lastUpdated`) VALUES (NULL, 'Ananda ', 'Utama', 'ananda_utama@y7mail.com', '0416039148', '1234567890', '1234567890', 'Technician', '144.965332', '-37.808927', '1', NOW());

INSERT INTO `registercar` (`carID`, `keyID`, `registerationID`, `nameAlias`, `time_stamp_last_serviced`, `description`, `userID`, `carImgURI`) VALUES (NULL, '1234567890', '1234567890', 'Jazzmine', NOW(), NULL, '1', NULL);

INSERT INTO `data` (`measurementID`, `temp`, `humidity`, `odometer`, `gps_long`, `gps_lati`, `battery_status`, `description`, `carID`, `userID`, `time_stamp`) VALUES (NULL, '16', '70', '100', '144.965332', '-37.808927', '100', NULL, '1', '1', NOW());

INSERT INTO `joblist` (`jobID`, `keyID`, `registerID`, `isJobActive`, `time_stamp_created`, `time_stamp_lastupdated`, `isJobApproved`, `description`, `carID`, `userID`) VALUES (NULL, '1234567890', '1234567890', '1', NOW(), NOW(), '1', NULL, '1', '1');